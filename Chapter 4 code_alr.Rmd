...
```{r priors}
#plot the priors
sample_mu<- rnorm(1e5,178,20) #sample 10k mu from normal distribution
sample_sigma <- runif(1e5,0,50)
prior_h <- rnorm(1e5 , sample_mu,sample_sigma) #combine uncertainty from both sample mu and sample sigma
hist(prior_h)
dens(prior_h)
```
...
d_grid <-
  ...
  expand(mu, sigma) #takes every possible value of mu and sigma from list and makes every possible combination
...
#likelihood is probability of the data given the model, parameters
dnorm(151.765, mean = 140, sd = 4)
dnorm(139.700, mean = 140, sd = 4) #small likelihood because distribution is so flat
```

```{r grid_posterior_execution}
post <- d_grid 
post$log_likelihood <- sapply( 1:nrow(post) , function(i) sum(dnorm( #sum not multiplication because log=TRUE
  d2$height , 
  mean=post$mu[i],
  sd=post$sigma[i],
  log=TRUE ) ) )
#less negative values have higher likelihood (because log = TRUE)

post<- post %>%
  mutate(prior_mu = dnorm(mu, mean = 178, sd  = 20, log = T), #prior probability that mean follows grid approx
         prior_sigma = dunif(sigma, min = 0, max = 50, log = T)) %>% #prior probability that sd follows grid approx
  mutate(unstd.posterior = log_likelihood + prior_mu + prior_sigma) %>% #un-standardized posterior (add up likelihoods)
  mutate(prob = exp(unstd.posterior - max(unstd.posterior))) #posterior probability - subtract not divide because log=TRUE
view(post)
```
...
R4.15 and R4.16
```{r view_posterior}
contour_xyz(post$mu, post$sigma , post$prob) #152 to 157 mean height is supported by model
...
ggplot(data=samples, aes(x=mu,y=sigma)) + geom_jitter() # look at grid approximation and draw parameter values when considering their posterior probability (higher probability means more points on the plot)

ggplot(samples,aes(x = sigma)) +
  geom_density(fill = "black", size = 0) #plot density of sigma values 
  
ggplot(samples,aes(x = mu)) +
  geom_density(fill = "black", size = 0) #plot density of mu values
...
print(stanfit_gauss)
pairs(stanfit_gauss,pars=c("mu", pars="sigma") ) #plot pairs of parameters

#gradient evaluation tells you how long you should expect the iteration to take
#rhat value tells you concordance of chains (larger than 1 tells you something isn't right, maybe run chains for longer)
...
m4.3_centered <- rstan::extract(stanfit_linear_centered, pars = c("alpha", "beta", "sigma"))
...
ggplot(d2,aes(x = weight, y = height)) +
  geom_abline(intercept = mean_params[1,5], #manually extract out intercept (and slope) from mean_params
