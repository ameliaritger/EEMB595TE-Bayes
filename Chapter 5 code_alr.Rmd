...
post <- posterior_samples(b5.3) %>% mutate(b_diff=b_Marriage_s-b_MedianAgeMarriage_s) %>% select(-lp__)
...
fitted(b5.3) %>%
  ...
  theme(panel.grid = element_blank())
```
A lot of overlap with the dashed line means our model is performing not-badly with our real data
